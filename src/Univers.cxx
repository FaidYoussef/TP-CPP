...
/**
 * @file Univers.cpp
 * @brief Implementation of the Univers class.
 */
...
void Univers::assignParticule(const Particule3D& particule, int nCellsX) {
    // Check if the particle belongs to the domain
    ...
    for (auto& cellule : cellules) {
        ...
        for (int i = static_cast<int>(part.size()) - 1; i >= 0; --i) {
            ...
            if (p.getPos().getX() < 0 || p.getPos().getX() > L1 ||
                ...
                p.getPos().getZ() <0  || p.getPos().getZ() > L3) {
...
void Univers::reassignCells() {
    ...
    for (auto& p : allParticules) {
        ...
        if (cellX >= 0 && cellX <= nCellsX && cellY >= 0 && cellY <= nCellsY) {
            ...
            std::cout << "Total cells in Y: " << nCellsY << std::endl;
            std::cout << "Particle X cell: " << cellX << " , Position X: " << p.getPos().getX() << std::endl;
            std::cout << "Particle Y cell: " << cellY << " , Position Y: " << p.getPos().getY() << std::endl;
